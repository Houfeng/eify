let _a = require('ntils'), final = _a.final, isArray = _a.isArray, copy = _a.copy, each = _a.each;
/**
 * 事件触发器基类
 */
let EventEmitter = /** @class */ (function () {
  /**
     * 构建一个一个事修的触发器对象
  ...
  EventEmitter.prototype.emit = function (name, data, canBubble, cancelAble) {
    ...
    let listeners = this._listeners_[name];
    if (!listeners)
      return;
